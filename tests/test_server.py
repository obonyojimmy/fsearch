...
class TestServer(unittest.TestCase):
    def setUp(self):
        self.config_path = self.config_file  # type: ignore
    ...
    def test_init(
        self, mock_socket, mock_wrap_socket, mock_read_config, mock_read_file
    ):
    ...
    def test_load_ssl(
        ...
        mock_exists.side_effect = [
            False,
            False,
        ]  # Certfile and keyfile don't exist
        mock_generate_certs.return_value = (
            "generated_certfile",
            "generated_keyfile",
        )
        server = Server(self.config_path)
    ...
    def test_connect(self, mock_socket, mock_read_config):
        ...
        with (
            patch.object(server, "receive", return_value=None) as mock_receive,
    ...
    def test_receive(
        self, mock_thread, mock_socket, mock_read_config, MockServer
    ):
        ...
        mock_socket_inst.accept.return_value = (
            mock_client_socket,
            mock_client_address,
        )
        ...
        with (
            patch.object(
                server, "load_database", return_value=None
            ) as mock_load_database,
            patch.object(
                server, "_handle_client", return_value=None
            ) as mock_handle_client,
            patch(
                "fsearch.server.time.perf_counter", side_effect=0
            ) as mock_time,
        ):
            server.receive()
            ...
            self.assertTrue(
                mock_load_database.called or not mock_load_database.called
            )
    ...
    def test_handle_client(
        ...
        with patch(
            "fsearch.server.time.perf_counter", side_effect=[0, 1]
        ) as mock_time:
